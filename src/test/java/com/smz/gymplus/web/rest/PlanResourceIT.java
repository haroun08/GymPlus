...
import static com.smz.gymplus.domain.PlanAsserts.*;
...
class PlanResourceIT {
    ...
    public static Plan createEntity(EntityManager em) {
        Plan plan = new Plan().name(DEFAULT_NAME).price(DEFAULT_PRICE);
        return plan;
    }
    ...
    public static Plan createUpdatedEntity(EntityManager em) {
        Plan plan = new Plan().name(UPDATED_NAME).price(UPDATED_PRICE);
        return plan;
    }
    ...
    void partialUpdatePlanWithPatch() throws Exception {
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertPlanUpdatableFieldsEquals(partialUpdatedPlan, getPersistedPlan(plan));
    }
