  ...
  return (
    <div>
      <h2 id="period-heading" data-cy="PeriodHeading">
        <Translate contentKey="gymPlusApp.period.home.title">Periods</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="gymPlusApp.period.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/period/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="gymPlusApp.period.home.createLabel">Create new Period</Translate>
          </Link>
        </div>
      </h2>
        ...
        {periodList && periodList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="gymPlusApp.period.id">Id</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('monthOccurrence')}>
                  <Translate contentKey="gymPlusApp.period.monthOccurrence">Month Occurrence</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('monthOccurrence')} />
                </th>
                <th className="hand" onClick={sort('dayOccurrence')}>
                  <Translate contentKey="gymPlusApp.period.dayOccurrence">Day Occurrence</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('dayOccurrence')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {periodList.map((period, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{period.monthOccurrence}</td>
                  <td>{period.dayOccurrence}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() => (window.location.href = `/period/${period.id}/delete`)}
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gymPlusApp.period.home.notFound">No Periods found</Translate>
            </div>
          )
        )}
      </div>
