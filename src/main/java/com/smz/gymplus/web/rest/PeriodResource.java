...
/**
 * REST controller for managing {@link com.smz.gymplus.domain.Period}.
 */
@RestController
...
public class PeriodResource {
    ...
    public ResponseEntity<Period> createPeriod(@RequestBody Period period) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/periods/" + period.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, period.getId().toString()))
            .body(period);
    }
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<Period> updatePeriod(@PathVariable(value = "id", required = false) final Long id, @RequestBody Period period)
    ...
    public ResponseEntity<Period> partialUpdatePeriod(
        ...
        Optional<Period> result = periodRepository
            ...
            .map(existingPeriod -> {
                if (period.getMonthOccurrence() != null) {
                    existingPeriod.setMonthOccurrence(period.getMonthOccurrence());
                }
                if (period.getDayOccurrence() != null) {
                    existingPeriod.setDayOccurrence(period.getDayOccurrence());
                }
